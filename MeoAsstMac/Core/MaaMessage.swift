...
extension MAAViewModel {
    private func processTaskChainMessage(_ message: MaaMessage) {
        ...
        let isCopilot = ["Copilot", "SSSCopilot", "ParadoxCopilot"].contains(info.taskchain)
...
extension MAAViewModel {
    private func processSubTaskExtraInfo(_ details: JSON) {
        ...
        case "CopilotListTaskFileFailed":
            if let fileName: String = try? info.details["file_name"] {
        ...
        case "CopilotBattleLeakDetected":
            logInfo(verbatim: "检测到漏怪，正在退出当前作战并重试")
        ...
        case "CopilotListLoadTaskFileSuccess":
            ...
            currentCopilotFileName = file.file_name
